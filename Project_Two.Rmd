...
The data is sourced from 10 distinct centers. Notably, a substantial proportion (64%) of the dataset is derived from a single center, while the remaining 36% is distributed among the other centers. This distribution disparity raises the potential for bias, particularly if the characteristics of the predominantly represented center significantly differ from those of the remaining centers. This lopsided representation may impact the generalizability and potential biases in the findings derived from this analysis.

Outliers are discernible within the dataset. Employing standardized data and employing a threshold of 3.29 for comparison, it is evident that outliers exist for variables encompassing birth weight, inspiratory pressure at 36 weeks, positive end-expiratory pressure (PEEP) at 36 weeks, and inspired oxygen levels at both time points.
...
tab_header(gt_table, title = 'Table 1. Differences by Tracheostomy Status')
```
...
#corrplot(cor_mat, method = "shade", type = "lower", tl.cex = 0.8, tl.col = "black", tl.srt = 45)
```
...
```{r}
forward_selection <- function(df, colnames, interactions) {
...
rownames(coefs_36_df) <- c("Prenatal Corticosteroids", "SGA", "Growth (from Birth)", "Inspired O2", "Medication for PH", "Invasive Positive Pressure")
colnames(coefs_36_df) <- c("Value")

coefs_44_df <- as.data.frame(avg_coefs_44[avg_coefs_44 != 0])
rownames(coefs_44_df) <- c("Cesarean Section", "Prenatal Corticosteroids", "Maternal Chorioamnionitis", "SGA", "Growth (from 36 Weeks)", "PEEP", "Medication for PH", "Invasive Positive Pressure")
colnames(coefs_44_df) <- c("Value")

kable(list(coefs_36_df, coefs_44_df), 
      caption = "Coefficients for Fixed Effect Models: 36 Weeks (Left) and 44 Weeks (Right)") %>% kable_styling(latex_options = "HOLD_position")
```

The addition of $\beta_{0j}$, the random intercept where j indicates center, resulted in the following two models.

$$
\begin{split}
E[Y_0] = \beta_{0j} + \beta_1 \times X_{\text{prenatal corticosteroids}} + \beta_2 \times X_{\text{SGA}} + \beta_3 \times X_{\text{growth(from birth)}} + \beta_4 \times X_{\text{inspired O2}} + \\\beta_5 \times X_{\text{medication for PH}} +  \beta_6 \times X_{\text{non-invasive positive pressure}} + \beta_7 \times X_{\text{invasive positive pressure}}
\end{split}
$$

for the 36 week time points and$$
\begin{split}
E[Y_0] = \beta_{0j} + \beta_1 \times X_{\text{cesarean section}} + \beta_2 \times X_{\text{prenatal corticosteroids}} + \beta_3 \times X_{\text{maternal chorioamnionitis}} + \beta_4 \times X_{\text{SGA}} + \\ \beta_5 \times X_{\text{growth (from 36 weeks)}} \beta_6 \times X_{\text{medication for PH}} + \beta_5 \times X_{\text{non-invasive positive pressure}} + \\ \beta_6 \times X_{\text{invasive positive pressure}} + \beta_7 \times X_{\text{PEEP}}
\end{split}
$$ for the 44 week time points.

Table 4 delineates the coefficients derived from the mixed effects models. The introduction of the random effect has induced alterations in the coefficients compared to the fixed effects models. Notably, for variables such as small for gestational age (SGA), medication for pulmonary hypertension (PH), and non-invasive positive pressure (exclusive to the 44-week model), the impact on the log odds of tracheostomy is now negative, contrasting with the positive effects observed in the fixed effects models.

```{r}
coef.mixed_36 <- as.data.frame(t(coef(mixed.36)$center[1,-1]))
colnames(coef.mixed_36) <- c("Value")
rownames(coef.mixed_36) <- c("Prenatal Corticosteroids", "SGA", "Growth (from Birth)",
                             "Inspired O2", "Medication for PH", 
                             "Non-Invasive Positive Pressure", "Invasive Positive Pressure")
#coef.mixed_36['Within-Center Variation', 'Value'] <- 0.02215

coef.mixed_44 <- as.data.frame(t(coef(mixed.44)$center[1,-1]))
colnames(coef.mixed_44) <- c("Value")
rownames(coef.mixed_44) <- c("Cesarean Section", "Prenatal Corticosteroids", 
...
#coef.mixed_44['Within-Center Variation', 'Value'] <- 0.02227

kable(list(coef.mixed_36, coef.mixed_44), 
      caption = "Coefficients for Random Effect Models: 36 Weeks (Left) and 44 Weeks (Right)") %>% kable_styling(latex_options = "HOLD_position")
```
...
model_validation <- function(test_data, coefs) {
  ...
  precision = recall = f1 = threshold = c()
...
val_df <- t(data.frame(week36 = as.numeric(val.36), week44 = as.numeric(val.44)))
rownames(val_df) <- c("Week 36 Model", "Week 44 Model")
colnames(val_df) <- c("Threshold", "Precision", "Recall", "F Score")
kbl(val_df, caption = 'Model Validation Metrics for Fixed Effect 36 and 44 Week Models', booktabs = T, escape = F) %>%
  kable_styling(latex_options="HOLD_position")
...
mixed_val_df <- t(data.frame(week36 = as.numeric(mixed_val.36), week44 = as.numeric(mixed_val.44)))
rownames(mixed_val_df) <- c("Week 36 Model", "Week 44 Model")
colnames(mixed_val_df) <- c("Threshold", "Precision", "Recall", "F Score")
kbl(mixed_val_df, caption = 'Model Validation Metrics for Mixed Effect 36 and 44 Week Models', booktabs = T, escape = F) %>%
  kable_styling(latex_options="HOLD_position")
```
...
#### Figure 2. Plot of Predicted Probabilities by Ventilation Status
