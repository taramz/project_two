...
Logistic regression models and mixed-effects models, acknowledging center-level variation, reveal consistent predictors in both 36 and 44-week models. Evaluation metrics favor the 44-week model, indicating superior predictive accuracy, precision, and recall. Surprisingly, the addition of a random intercept for center minimally affects model performance on validation data, suggesting limited center-level variation. All four models performed well on the validation data, though the 44-week model with a mixed effect yielded the highest AUC (0.89) and lowest Brier score (0.10).
...
The data derived from diverse medical centers manifests noteworthy disparities, with Center 4 notably lacking recorded measurements at the 44-week interval. Furthermore, a substantial proportion of centers exhibit a heightened prevalence of missing data points for measurements obtained at the 44-week interval compared to those collected at the 36-week mark. Specifically, approximately 43% of observations contain missing values for all measurements collected at the 44-week mark. This could be due to the infants being discharged prior to the 44-week mark. In fact, Table 4 shows that infants with data missing at the 44 week mark have less tracheostomy prevalence than those with data at the 44 week mark. Either way, these patterns of missing data warrant thorough investigation and, if deemed appropriate, consideration of imputation techniques.
...
# Look at tracheostomy prevalence between infants with data at 44 weeks and those without it
trach_discharged <- data %>%
  filter(is.na(peep_cm_h2o_modified.44) & is.na(weight_today.44) & is.na(inspired_oxygen.44) &
           is.na(ventilation_support_level_modified.44) & is.na(p_delta.44)) %>%
  summarise(n = sum(Trach)/n()) %>%
  pull(n)

trach_not_discharged <- data %>%
  filter(!is.na(peep_cm_h2o_modified.44) & !is.na(weight_today.44) & !is.na(inspired_oxygen.44) &
           !is.na(ventilation_support_level_modified.44) & !is.na(p_delta.44)) %>%
  summarise(n = sum(Trach)/n()) %>%
  pull(n)

trach_prevalence <- t(data.frame(discharged = trach_discharged,
                                  not_discharged = trach_not_discharged))
colnames(trach_prevalence) <- c('Tracheostomy Proportion')
rownames(trach_prevalence) <- c('Missing 44 Week Data', 'Not Missing 44 Week Data')

kable(trach_prevalence, booktabs = T, escape = F, caption = 'Tracheostomy Prevalence Between Infants With and Without Data Collected at 44 Weeks') %>%
  kable_styling(latex_options = 'HOLD_position')
...
As previously outlined, two distinct models were developed: one focused on predictors gathered at 36 weeks, and the other on predictors obtained at 44 weeks. As mentioned earlier, tracheostomy prevalence between infants with data only at 36 weeks and those with data at 44 weeks is different, indicating that the 44 week model may be better at predicting tracheostomy, since the prevalence is higher. A combined approach involving best subsets and 10-fold cross-validation was employed for variable selection in each model to enhance model generalizability, mitigate overfitting, and yield more robust estimates. This iterative procedure transpired across the five imputed datasets, with subsequent calculation of average coefficients for the selected variables. The resultant averages were then utilized as the definitive covariates in the subsequent model validation phase.

While the consideration of interaction terms was intrinsic to the model selection process, the best subsets method exhibited a propensity for favoring interaction terms over main effects when all potential interaction terms were included. Consequently, the decision was made to exclusively focus on interactions with pulmonary hypertension, as research suggests pulmonary hypertension is associated with severe maternal complications and high mortality [@pieper2011pregnancy]. Therefore, it is reasonable to assume there might be some significant interactions between pulmonary hypertension and the other respiratory parameters.

```{r}
forward_selection <- function(df, colnames, week) {
  ...
  # Include med_ph as interaction terms
  if (week == 36) {
    x.ord <- model.matrix(Trach ~ . + inspired_oxygen.36:med_ph.36 + p_delta.36:med_ph.36 +
                            peep_cm_h2o_modified.36:med_ph.36 + 
                            ventilation_support_level.36:med_ph.36, data = df[,colnames])[,-1]
  } else {
    x.ord <- model.matrix(Trach ~ . + inspired_oxygen.44:med_ph.44 + p_delta.44:med_ph.44 +
  ...
  abess_fit <- abess(x.ord, y.ord, family = "binomial", 
                     tune.type = "cv", nfolds = 10, seed = 1, fit.intercept = TRUE)
...
coefs_1.36 <- forward_selection(train_imp[[1]], colnames_36, 36)
coefs_2.36 <- forward_selection(train_imp[[2]], colnames_36, 36)
coefs_3.36 <- forward_selection(train_imp[[3]], colnames_36, 36)
coefs_4.36 <- forward_selection(train_imp[[4]], colnames_36, 36)
coefs_5.36 <- forward_selection(train_imp[[5]], colnames_36, 36)

coef_36 <- cbind(coefs_1.36, coefs_2.36, coefs_3.36,
                 coefs_4.36, coefs_5.36) 
avg_coefs_36 <- apply(coef_36, 1, mean)

coefs_1.44 <- forward_selection(train_imp[[1]], colnames_44, 44)
coefs_2.44 <- forward_selection(train_imp[[2]], colnames_44, 44)
coefs_3.44 <- forward_selection(train_imp[[3]], colnames_44, 44)
coefs_4.44 <- forward_selection(train_imp[[4]], colnames_44, 44)
coefs_5.44 <- forward_selection(train_imp[[5]], colnames_44, 44)
...
mixed.36 <- lmer(Trach ~ prenat_ster + sga + growth_36 + inspired_oxygen.36 + med_ph.36 + ventilation_support_level.36 + med_ph.36 * ventilation_support_level.36 + (1 | center), 
            data = test_data_long)
predict.36 <- predict(mixed.36)

mixed.44 <- lmer(Trach ~ del_method + prenat_ster + com_prenat_ster +  mat_chorio + 
                   gender + sga + growth_44 + peep_cm_h2o_modified.44 + med_ph.44 + 
                   ventilation_support_level_modified.44 + p_delta.44:med_ph.44 + 
                   peep_cm_h2o_modified.44:med_ph.44 + 
                   med_ph.44:ventilation_support_level_modified.44 + (1 | center), 
                 data = test_data_long)
...
Analysis of Table 5 reveals a positive effect on the log odds of receiving a tracheostomy for all covariates in both models. Specifically, according to the 36-week model, infants exposed to prenatal corticosteroids are 1.88 times more likely to undergo tracheostomy. Invasive positive pressure is the main effect that yields the largest odds ratio, signifying a substantial difference between infants receiving this respiratory support and those devoid of respiratory assistance. Despite the anticipated association between being classified as SGA and tracheostomy, SGA infants are 1.08 times more likely to undergo tracheostomy than their non-SGA counterparts.

Within the 44-week model, invasive positive pressure emerges with the highest odds ratio. Unexpectedly, a pronounced difference in the odds of tracheostomy is observed for infants administered medication for PH compared to the 36-week model, as the main effect is considered significant in this model, but not the 36 week model. Additionally, maternal chorioamnionitis surfaces as a predictor in the 44-week model, despite its absence in the 36-week model. Across both models, we see the interaction term between non-invasive positive pressure and medication for PH emerge as a significant predictor of receiving a tracheostomy.

```{r}
# Create tables containing coefficients for both fixed effects models
coefs_36_df <- as.data.frame(avg_coefs_36[avg_coefs_36 != 0])
coefs_36_df$col <- c("Prenatal Corticosteroids", "SGA", "Growth (from Birth)", "Inspired O2", "Invasive Positive Pressure", "Non-Invasive Positive Pressure x Medication for PH")
colnames(coefs_36_df) <- c("Value", "Column")
rownames(coefs_36_df) <- c()

coefs_44_df <- as.data.frame(avg_coefs_44[avg_coefs_44 != 0])
coefs_44_df$col <- c("Cesarean Section", "Prenatal Corticosteroids", "Complete Prenatal Steroids", "Maternal Chorioamnionitis", "Gender", "SGA", "Growth (from 36 Weeks)", "PEEP", "Medication for PH", "Non-Invasive Positive Pressure", "Invasive Positive Pressure", "Peak Inspiratory Pressure x Medication for PH", "PEEP x Medication for PH", "Invasive Positive Pressure x Medication for PH", "Non-Invasive Positive Pressure x Medication for PH" )
colnames(coefs_44_df) <- c("Value", "Column")
rownames(coefs_44_df) <- c()
```

From Table 5, we can also see that the introduction of the random effect has induced alterations in the coefficients compared to the logistic regression effects models. Notably, for variables such as small for gestational age (SGA), and medication for pulmonary hypertension (exclusive to the 44-week model), the impact on the log odds of tracheostomy is now negative, contrasting with the positive effects observed in the logistic regression models.
...
coef.mixed_36$col <- c("Prenatal Corticosteroids", "SGA", "Growth (from Birth)",
                       ...
                       "Non-Invasive Positive Pressure x Medication for PH")
...
coef.mixed_44$col <- c("Cesarean Section", "Prenatal Corticosteroids", 
                       "Complete Prenatal Steroids", "Maternal Chorioamnionitis", 
                       "Gender", "SGA", "Growth (from 36 Weeks)", "PEEP", 
                       "Medication for PH", "Non-Invasive Positive Pressure", 
                       "Invasive Positive Pressure", 
                       "Peak Inspiratory Pressure x Medication for PH", 
                       "PEEP x Medication for PH", 
                       "Invasive Positive Pressure x Medication for PH",
                       "Non-Invasive Positive Pressure x Medication for PH")
...
model_comparison <- coefs_36_df %>%
  ...
  dplyr::select(Column, value_1 = Value.x, value_2 = Value.y) %>%
  full_join(coef.mixed_36, by = 'Column') %>%
  dplyr::select(Column, value_1, value_2, value_3 = Value) %>%
  full_join(coef.mixed_44, by = 'Column') %>%
  dplyr::select(Column, value_1, value_2, value_3, value_4 = Value)
...
Model evaluation encompassed the Brier score, precision, recall, and the F-score, with the optimal threshold determined by the ROC curve and documented in Table 6. Notably, a discernible trend suggests higher predictive accuracy in the 44-week model compared to the 36-week model. This observation extends to improved precision and recall metrics, coupled with a higher optimal threshold for predictions in the 44-week model. Additionally, a lower Brier score is noticed when comparing both 44 week models to their 36 week counterparts. Similarly, the models with the random intercept have better calibration measures than the models without the random intercept.

Table 6 also delineates the mixed effects model's performance on validation data. Strikingly, though the mixed effect 44 week model performs best, the introduction of the random intercept yields only a small change in model performance. This outcome suggests that the anticipated substantial variation between centers might be less pronounced than initially envisaged. To further explore this, the Intra-Class Correlation Coefficient (ICC) was computed using the formula:
...
Calculated ICCs for the 36-week and 44-week mixed effects models stand at 0.2056 and 0.2446, respectively. To justify the adoption of a random intercept, an ICC greater than 0.5 is typically sought, indicating substantial within-group variability amenable to capture by a random effect.

```{r}
# Model Validation
# Use ROC curve to get the best threshold
names(avg_coefs_36)[20:21] <- c('ventilation_support_level.361:med_ph.36', 
                                'ventilation_support_level.362:med_ph.36')

names(avg_coefs_44)[20:21] <- c('ventilation_support_level_modified.441:med_ph.44',
                                'ventilation_support_level_modified.442:med_ph.44')

get_metrics <- function(data, model_coefs, week) {
  ...
  if (week == 36) {
    x_vars <- model.matrix(Trach ~  . + med_ph.36:(.), data = data)[,-1]
  } else if (week == 44){
    x_vars <- model.matrix(Trach ~  . + med_ph.44:(.), data = data)[,-1]
  }
...
model_validation <- function(test_data, coefs, week) {
  ...
  for (i in 1:length(test_data)) {
    if (week == 36) {
      metrics <- get_metrics(test_data[[i]], coefs, 36)
      ...
      metrics <- get_metrics(test_data[[i]], coefs, 44)
      precision <- c(precision, metrics$precision) 
      recall <- c(recall, metrics$recall)
      f1 <- c(f1, metrics$f1)
      threshold <- c(threshold, metrics$best_threshold) 
...
val.36 <- model_validation(test_imp, avg_coefs_36, 36)
val.44 <- model_validation(test_imp, avg_coefs_44, 44)

val_df <- t(data.frame(week36 = round(as.numeric(val.36), 2), 
                       week44 = round(as.numeric(val.44), 2)))
...
library(gridExtra)
x_vars_36 <- model.matrix(Trach ~ . + med_ph.36:(.), data = test_data_long)
x_vars_44 <- model.matrix(Trach ~ . + med_ph.44:(.), data = test_data_long)
x_vars_44 <- x_vars_44[,names(avg_coefs_44)]
x_vars_36 <- x_vars_36[,names(avg_coefs_36)]
